...
func TestTradingParShouldCalculateVWAPFromTwoTradingPairs(t *testing.T) {
	tp := TradingPair{WindowSize: 2}
	err := tp.Add(Price{Price: 4.0, Size: 2.0}) // (8 + 48) / 8
	assert.Nil(t, err)
	err = tp.Add(Price{Price: 8.0, Size: 6.0})
	assert.Nil(t, err)
...
func TestTradingPairShouldRemoveTheOldestValue(t *testing.T) {
	tp := TradingPair{WindowSize: 2}
	err := tp.Add(Price{Price: 4.0, Size: 2.0}) // will be ignored, because window length is 2
	assert.Nil(t, err)
	err = tp.Add(Price{Price: 8.0, Size: 6.0}) // (48 + 50) / 11
	assert.Nil(t, err)
	err = tp.Add(Price{Price: 10.0, Size: 5.0})
	assert.Nil(t, err)
...
func TestShouldNotAllowNegativeNumberOnPrice(t *testing.T) {
	tp := TradingPair{WindowSize: 2}
	err := tp.Add(Price{Price: -4.0, Size: 2.0})
	assert.NotNil(t, err)
}
